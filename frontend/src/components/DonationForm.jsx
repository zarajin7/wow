...
const DonationPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchCampaigns = async () => {
      try {
        const response = await get("http://localhost:8000/api/campaigns/");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Extract the new collected amount from the response
      const newCollectedAmount = response.data.collected;

      // Update the campaign progress
      await axios.patch(`http://localhost:8000/api/campaigns/${selectedCampaign}/`, {
        collected: newCollectedAmount,
      });

      // Generate QR code URL for the payment
      setQrCodeUrl(`http://localhost:8000/api/payment/${response.data.id}`);

      alert("Donation successful! Please scan the QR code to complete the payment.");
    } catch (error) {
